...
fn group_bounds_is_origin_if_no_members() {
    ...
    assert_bounds(g.clone(), (pos_inf, pos_inf, pos_inf), (neg_inf, neg_inf, neg_inf));
}

#[allow(non_snake_case)]
#[test]
fn group_minima_and_maxima_are_dictated_by_its_members_bounds() {
    assert_bounds(group(vec![unit_sphere(), cylinder(CylKind::Open, -5.0, 2.0)])
                  , (-1.0, -5.0, -1.0), (1.0, 2.0, 1.0));
    assert_bounds(group(vec![unit_sphere(), cube()])
