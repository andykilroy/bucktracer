...
fn main() -> Result<()> {
    ...
    for i in 0..12 {
        let ith_point = identity().rotate_z(-(f64::from(i)) * (PI / 6.0)).mult(zero);
...
fn plot(
    ...
    // Take world coords, and centre them around (0,0);
    // Squashes the coordinates so they fit in
    // bounds of box (-1/2, -1/2), (1/2, 1/2);
    // Reflect in y, otherwise the
    // image will appear flipped from top to bottom;
    // Translate so coords are within box (0,0), (1,1)
